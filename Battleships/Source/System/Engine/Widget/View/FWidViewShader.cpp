...
bool FWidViewShader::Initialize()
{
	...
	result = CreateShaderFromFile("Source\\System\\Engine\\Widget\\View\\FWidViewShader.vs", GL_VERTEX_SHADER, m_VertexShader);
	if (!result)
	{
		return false;
	}
	...
	result = CreateShaderFromFile("Source\\System\\Engine\\Widget\\View\\FWidViewShader.fs", GL_FRAGMENT_SHADER, m_FragmentShader);
	if (!result)
	{
		return false;
	}
...
void FWidViewShader::CreateViewMesh()
{
	struct WidgetVertex
	{
		WVector3 position;
...
void FWidViewShader::Render(float _elapsedTime, FCamera* _camera, FGraphic* _graphicContext)
{
	...
	for (int i = 0; i < m_RenderQueue.Size(); i++)
	{
		...
		transformMatrix.BuildTransform(m_RenderQueue[i]->GetPosition(), WVector3(0, 0, m_RenderQueue[i]->GetRotation()), WVector3(m_RenderQueue[i]->GetSize().x, m_RenderQueue[i]->GetSize().y, 1));
		...
		glBindTexture(GL_TEXTURE_2D, m_RenderQueue[i]->Texture()->GetHandle());

		// Set the texture in the pixel shader to use the data from the first texture unit.
		location = glGetUniformLocation(m_ShaderProgram, "viewTexture");
		if (location == -1)
		{
			return;
		}
...
void FWidViewShader::AddObject(void* _objectPtr)
{
	// Cast the object
	FWidView* widget = (FWidView*)_objectPtr;
