...
bool FLightShader::Initialize()
{
	...
	result = CreateShaderFromFile("Source\\Core\\Renderable\\Light\\FLightShader.vs", GL_VERTEX_SHADER, m_VertexShader);
	if (!result)
	{
		return false;
	}
	...
	result = CreateShaderFromFile("Source\\Core\\Renderable\\Light\\FLightShader.fs", GL_FRAGMENT_SHADER, m_FragmentShader);
	if (!result)
	{
		return false;
	}
...
void FLightShader::CreateLightMesh()
{
	struct WidgetVertex
	{
		WVector3 position;
...
void FLightShader::Render(float _elapsedTime, FCamera* _camera, FGraphic* _graphicContext)
{
	...
	for (auto it = m_RenderQueue.begin(); it != m_RenderQueue.end(); it++)
	{
		...
		FLight* light = it->second;

		// Get the actor linked to this flipbook
		FActor* owner = light->GetOwner();
		...
		glBindTexture(GL_TEXTURE_2D, light->Texture()->GetHandle());
		...
		glUniform4f(location, light->Color().x, light->Color().y, light->Color().z, light->Alpha());
...
void FLightShader::AddObject(void* _objectPtr)
{
	// Cast the object
	FLight* light = (FLight*)_objectPtr;

	// Insert into the map
	m_RenderQueue.insert(std::pair<int, FLight*>(light->GetOwner()->GetDepthPlane(), light));
}